...
import { Check, Copy, Dumbbell, RefreshCw, AlertTriangle, Coins, DoorOpen } from "lucide-react";
...
/** Catraca física da própria academia (torniquete Henry + leitor Control iD
 *  linha EVO). Diferente de Gympass/TotalPass, não é plataforma parceira: não
 *  tem status/homologação nem repasse — só o endpoint que decide a passagem em
 *  tempo real e o segredo que autentica o equipamento. */
function CatracaFisicaBloco({
  slug,
  secretMascarado,
  isDemo = false,
}: {
  ...
  const url = `${origem || "https://SEU-DOMINIO"}/api/webhook/catraca/${slug}`;

  function rotacionar() {
    if (
      !window.confirm(
        "Gerar novo segredo para a catraca?\n\nO segredo atual deixará de funcionar imediatamente. Você precisará atualizá-lo no equipamento/integrador (Exacta Cards) após isso."
      )
    )
      return;
    ...
    setErro(null);
    startRotar(async () => {
      const res = await rotarSecretCatraca(slug);
      if (res.erro) setErro(res.erro);
    });
  }
    ...
    return (
      <div className="surface relative space-y-4 rounded-2xl p-5 opacity-60">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-white">Catraca física</h2>
        </div>
        <div className="absolute inset-0 z-10 flex items-center justify-center rounded-2xl bg-ink-900/70">
          <p className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-2 text-sm font-medium text-amber-300">
            Ação indisponível no ambiente de demonstração.
  ...
  return (
    <div className="surface space-y-4 rounded-2xl p-5">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">
            Torniquete Henry + leitor facial/cartão Control iD (linha EVO)
          </p>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-ink-700 bg-ink-800/50 p-4 text-sm text-slate-300">
        ...
        <ol className="list-decimal space-y-1.5 pl-5 text-slate-400">
          <li>
            Entregue ao instalador da catraca (Exacta Cards) a{" "}
            <strong>URL do webhook</strong> acima e o <strong>segredo</strong> gerado aqui.
          </li>
          <li>
            Configure o leitor Control iD no <strong>modo online</strong>: a cada
            identificação (rosto ou cartão) ele consulta essa URL com o cabeçalho{" "}
            <code className="rounded bg-ink-900 px-1 py-0.5 text-xs">
              Authorization: Bearer &lt;segredo&gt;
            </code>
            .
          ...
          <li>
            O app responde <code className="rounded bg-ink-900 px-1 py-0.5 text-xs">liberado: true/false</code>{" "}
            aplicando a mesma <strong>política de inadimplência</strong> da recepção — e
            registra a passagem no histórico de acessos.
          </li>
          <li>
            Cadastre cada aluno com o <strong>código de matrícula (cartão)</strong> ou o{" "}
            <strong>CPF</strong> usado no equipamento — é assim que a passagem é vinculada
            ao aluno.
          </li>
        </ol>
      </div>
      ...
      <button onClick={rotacionar} disabled={pendingRotar} className="btn-ghost text-xs">
        <RefreshCw className={`h-3.5 w-3.5 ${pendingRotar ? "animate-spin" : ""}`} />
        {pendingRotar ? "Gerando..." : "Gerar novo segredo"}
      </button>
    </div>
...
export default function Integracoes({
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-4 text-sm text-slate-300">
        <div className="flex items-start gap-3">
          <DoorOpen className="mt-0.5 h-5 w-5 flex-none text-indigo-300" />
          <div>
            <p className="font-medium text-white">Catraca física da academia</p>
            <p className="mt-1 text-slate-400">
              Conecte o torniquete Henry com leitor Control iD (linha EVO): cada passagem
              é registrada aqui e, no modo online, o app libera ou bloqueia a entrada em
              tempo real pela mesma política de inadimplência da recepção.
            </p>
          </div>
        </div>
      </div>
      ...
      <CatracaFisicaBloco
        slug={slug}
        secretMascarado={catracaSecretMascarado}
        isDemo={isDemo}
      />
    </div>
  );
