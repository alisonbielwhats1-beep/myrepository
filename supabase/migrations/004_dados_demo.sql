...
begin
  ...
  perform set_config('gymflow.demo_academia_id', v_academia_id::text, false);
...
select (current_setting('gymflow.demo_academia_id')::uuid), v.nome, v.descricao, v.valor, v.rec, true
from (values
...
where not exists (
  ...
  where academia_id = (current_setting('gymflow.demo_academia_id')::uuid)
);
...
select
  (current_setting('gymflow.demo_academia_id')::uuid),
  t.nome,
  t.cargo,
...
where a.academia_id = (current_setting('gymflow.demo_academia_id')::uuid)
...
from (
  select
    (select id from public.alunos where academia_id = (current_setting('gymflow.demo_academia_id')::uuid) and matricula_codigo = 'DEMO-003') as aluno_id,
    'Whey Protein 900g' as produto, 149.90 as valor, (current_date - interval '2 months')::date as data
  union all
  select
    (select id from public.alunos where academia_id = (current_setting('gymflow.demo_academia_id')::uuid) and matricula_codigo = 'DEMO-007'),
    'Camiseta dry-fit', 69.90, (current_date - interval '5 months')::date
  union all
  select
    (select id from public.alunos where academia_id = (current_setting('gymflow.demo_academia_id')::uuid) and matricula_codigo = 'DEMO-010'),
    'Coqueteleira', 35.00, (current_date - interval '9 months')::date
  union all
  select
    (select id from public.alunos where academia_id = (current_setting('gymflow.demo_academia_id')::uuid) and matricula_codigo = 'DEMO-013'),
    'Luva de treino', 59.90, (current_date - interval '1 months')::date
  union all
  select
    (select id from public.alunos where academia_id = (current_setting('gymflow.demo_academia_id')::uuid) and matricula_codigo = 'DEMO-016'),
...
join public.alunos a
  ...
  and a.academia_id = (current_setting('gymflow.demo_academia_id')::uuid);
...
join public.alunos a on a.id = tr.aluno_id and a.academia_id = (current_setting('gymflow.demo_academia_id')::uuid)
join (values
